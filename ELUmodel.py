...
elu_model_training_history = elu_model.fit(
    x_train_enc_rest,
    y_train_enc_rest,
    batch_size = 512, 
    epochs = 150, 
...
loss = elu_model_training_history.history['loss']
val_loss = elu_model_training_history.history['val_loss']

plt.figure()
plt.plot(elu_model_training_history.history['loss'], label="Training Loss")
plt.plot(elu_model_training_history.history['val_loss'], label="Validation Loss")
plt.ylabel('Loss')
plt.xlabel('Epochs')
plt.title("model ELU relu  ")
plt.legend()
plt.show()
...
elu_dropout_l2_model = keras.Sequential([
    layers.InputLayer(input_shape=(10000,)),
...
elu_dropout_l2_model.compile(
    loss='categorical_crossentropy',
    optimizer=SGD(learning_rate=0.05),
    metrics=['accuracy']
)

elu_dropout_l2_history = elu_dropout_l2_model.fit(
    x_train_enc_rest,
    y_train_enc_rest,
    batch_size=512,
    epochs=150,
...
plt.figure()
plt.plot(elu_dropout_l2_history.history['loss'], label="Training Loss")
plt.plot(elu_dropout_l2_history.history['val_loss'], label="Validation Loss")
plt.ylabel('Loss')
plt.xlabel('Epochs')
plt.title("ELU + L2 Regularisation + Dropout")
plt.legend()
plt.show()
